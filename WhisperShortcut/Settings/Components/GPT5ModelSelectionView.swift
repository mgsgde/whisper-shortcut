...
/// Wiederverwendbare Komponente für die GPT-5-Modellauswahl
struct GPT5ModelSelectionView: View {
  let title: String
  ...
  var body: some View {
    VStack(alignment: .leading, spacing: SettingsConstants.internalSectionSpacing) {
      SectionHeader(
        title: title,
        subtitle: "Choose between GPT-5 models for prompt processing"
      )
...
#if DEBUG
  struct GPT5ModelSelectionView_Previews: PreviewProvider {
    static var previews: some View {
      @State var selectedModel: GPTModel = .gpt5Mini

      GPT5ModelSelectionView(title: "GPT-5 Model", selectedModel: $selectedModel)
        .padding()
        .frame(width: 600)
    }
  }
