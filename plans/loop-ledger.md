...
| --- | -------- | ---------------------------- | ------ | --- | ----- |
| L12 | Queue rows must be unambiguous to every reader: `queue-edit.py` rejects a `\|` (or any pipe) inside a cell at write time, and `tick.sh` mails when `grep` finds a `BUILD`/`OPEN` row that the picker did not return — the two readers disagreeing is the finding. Do not "fix" the picker with `$(NF-3)`; that breaks the same way from the other end | Row 6 promoted to BUILD 2026-09-09 00:23; its falsifier cell contains `\|`, so `run-implementer.sh:211-213` (`awk -F'|'`, `$6`/`$7`) never matched it. 174 consecutive ticks (09-09 00:23 → 09-19 10:21) printed "no BUILD/OPEN row" while a released row waited. Same defect class sabaki fixed 2026-09-13 (`704563571`, "compare the status WORD") | built | 2 | Effort S. Falsifier: row 6 builds within 24 h of the fix landing, and a forced row with a piped cell is refused by `queue-edit.py`. Built 2026-09-19 → PR #74 (tooling/loops-deliver, 53e5010). Review fix fb9663c: the groomer rewrites pipes in a proposal to ` / ` and marks the Source cell, so the refusal never aborts a groom pass |
| L11 | A digest is complete or the run FAILED: every job script checks size ≥ 2 KB, the required sections (`## Weglassen`, `## Self-answered`, and for growth a ledger entry for the run's ID), no "provisional" marker, and — for the loops that hand proposals over — that the proposal file exists (empty `[]` allowed). Plus: `scripts/loop-prompt.sh` forbids subagents/delegates in scheduled runs, or every job exports `CLAUDE_CODE_PRINT_BG_WAIT_CEILING_MS=0` — pick one, say which | G3 2026-09-19: the run delegated ASC reads to a subagent, `claude -p` killed it at the 600 s background ceiling, and `growth-review-job.sh:220` (`[ ! -f "$DIGEST" ]`) mailed a 253-byte "provisional" stub under "Nothing to do". No G3 ledger entry, no proposal file. The 09-05 run worked only because it did not delegate | built | 2 | Effort S. Falsifier: a forced stub run (`--dry-run` with a 100-byte digest) mails FAILED; the next scheduled growth run with `asc` slow completes with a full digest and a G-entry. Built 2026-09-19 → PR #74 (tooling/loops-deliver, 53e5010). Helper verified against fixtures; the job-level falsifier (a forced stub run mails FAILED) still needs a scheduled run. Review fix 796d7e2: the provisional check is a line-start marker (`^\s*\(?provisional`), not an anywhere-match — the 09-19 stub still fails it, a digest that discusses the stub passes. |
| L10 | "Delivered = committed" binds the loops too: each job commits its own ledger/gap/digest rows (explicit paths, on `main` or a `loop/<job>-<date>` branch the tick merges), so the groomer, the health mail and the next run read the same state as the working copy | I4–I9 and instrumentation gaps #9/#10 exist only in the uncommitted working copy (last ledger commit `ab15826`, 2026-09-05); the groomer reads `origin/main`, found no OPEN gap and filed both instrumentation proposals (rows 5, 8) as `ASK` instead of `BUILD`. Parent: `business/growth-reviews/2026-09-05`, `-09-19` untracked. One `git checkout -- plans/` deletes two weeks of usage-review output | built | 2 | Effort S–M. Falsifier: after the next usage run `git status --porcelain plans/ ../business/` is empty, and the next instrumentation proposal that names an OPEN gap lands `BUILD`, not `ASK`. Built 2026-09-19 → PR #74 (tooling/loops-deliver, 53e5010) |
| L9 | Add Status `WAITING` to the implementer queue: a row whose falsifier says "build only once X is measured" names its condition and drops out of the health mail's desk list and the runner's reach; it returns to `OPEN` when the condition holds — port of sabaki `queue-edit.ts` 2026-09-10 | Sabaki measured 3 of 18 desk rows on 2026-09-10 as waiting on a clock, printed as decisions; here `health-report.py` filters ASK+OPEN the same way, so the same misprint is one measurement-gated row away | proposed | — | Effort S (`queue-edit.py` PARK_STATUSES + `health-report.py` filter + queue header table). Falsifier: the first health mail after a `WAITING` row exists lists it under counts, not decisions; if no row ever needs it within 2 months, drop L9 |
| L8 | Port sabaki's falsifier bookkeeping: every shipped user-facing change carries a machine-readable falsifier (marker in the ledger row or a `<!-- falsifier: -->` comment in `plans/*.md`) with `path`, `window`, `metric`, `onFail`; a daily deterministic check starts the clock at the confirmed release (`asc versions list` / GitHub release / local rebuild), mails when a window closes, and a gated grading pass settles the bet and files `onFail=remove` rollbacks as `VETO` rows | Here a falsifier is a prose cell a human reads at the next loop run — I1 has been `shipped` since 2026-08-03 and is still not `measured` — nobody owns the clock. Sabaki's meta-log: "the whole thing stalls at the reminder" until grading was automated 2026-09-07 | proposed | — | Effort L — a design (`plans/active/`), not a copy: deploy check differs (App Store version vs `/api/version`), population is one operator, no `docs/*.md` marker sweep exists. Falsifier: within 60 days of building it, ≥1 ledger row is graded PASS/FAIL by the machine with the number in the mail, and the median proposed→graded lag drops below the current ∞ |
| L7 | Every scheduled loop digest must name one **Weglassen** (deletion candidate: a skill section, job flag, ledger row, or digest habit to remove) — port Sabaki's mandatory deletion line | Accumulation without pruning: four loops only ever add rows/checks; no run is forced to shrink the machine. Measured: 0 deletion candidates across all digests since 2026-08-03 | applied | 1 | Falsifier: by the next Architect run, ≥3 of the four loop families have a Weglassen line in their newest digest; if still 0, drop L7 as theatre. **Applied 2026-09-12** (manual convergence pass, user-approved): `scripts/loop-prompt.sh` appends a mandatory `## Weglassen` section to all four job prompts. **Run 2 (2026-09-19): TOO EARLY** — usage 09-14 has it; growth 09-19 is a stub; model-audit and sales have not run since. 1 of 4, grade at run 3 |
...
| --- | ---- | ------------------ | ------------- |
| 2 | 2026-09-19 | Loops measure well, deliver little — 2 autonomous PRs in 7 weeks; finding→build path broken in 3 silent places (stub digest mailed as success, row 6 unpickable, ledgers uncommitted). Interactive, on the operator's question. | L10, L11, L12; L2 superseded; L7 TOO EARLY |
| — | 2026-09-12 | Manual convergence pass with sabaki (not a scheduled run): layout, shared unattended rules, cadence gates, claim reproduction; the 2026-09-06 scheduled run was swallowed by the 20-day gate. | L8, L9; L7 applied |
