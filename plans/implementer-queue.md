...
| 3 | improvement-plan F15 (parked 2026-09-02) | Streaming local Dictate: admit Whisper in `DictateStreamingSession.makeIfEligible` so Offline Mode transcribes chunks while recording, with the existing merged-WAV fallback. **Do not add a Parakeet-class model in this row.** See `plans/active/streaming-dictate.md` slice 4. **Note (2026-09-03):** the shipped streaming leaves a fixed ~3 s floor per call that is Whisper's 30 s-padded encoder, not this row's scope — do not re-open this row to chase it, and do not propose a smaller `dictateChunkMinDuration` as the fix (measured: it cannot help). | A ≥20 s Offline Whisper dictation logs `SPEED: STREAMING-DICTATE: Transcript ready` and stop-to-clipboard is closer to last-chunk time than to full-file Whisper; a failed in-flight chunk still pastes via the merged-WAV fallback. Falsified if WhisperKit hangs/OOM during recording, or if chunk seams hallucinate after F14's peak gate. | BUILD | BRANCH claude/local-speech-to-text-optimize-0da93d |  |
| 4 | model-audit 2026-09-03 — Recommended migrations (seeded by hand: that run predates the proposal sidecar) | Add gemini-3.8-flash as a PromptModel and TranscriptionModel case and make it the default for Chat and Smart Improvement; point gemini35Flash.chatReplacement and gemini36Flash.chatReplacement at gemini37Flash, and move the migrateLegacyPromptRawValue pointers for gemini-2.5-flash and gemini-3-flash-preview from 3.5-flash to 3.7-flash so nobody is migrated onto a hidden model (SettingsConfiguration.swift, TranscriptionModels.swift, ChatModelCommandResolver.swift). Do NOT set gemini37Flash.chatReplacement = .gemini38Flash — the audit's own probe measured 3.8 slower, and that decision waits for an interleaved latency run. | On the branch build: a Chat message and a Smart Improvement run both resolve to gemini-3.8-flash and return HTTP 200 (`bash scripts/logs.sh -t 5m` shows the model ID with no fallback), and the /3.8 chat command selects it. Supersession has two distinct paths and both must land on a chat-selectable model: a slot stored on gemini-3.5-flash or gemini-3.6-flash is lifted ONCE to gemini-3.8-flash by the one-shot didMigrateDefaultsTo38Flash migration, which matches on the RESOLVED model rather than the raw string; after that flag is spent, a 3.5/3.6 value written later resolves to gemini-3.7-flash via chatReplacement at load time. Falsified if any of those still sends 3.7 where 3.8 is the default, errors, or leaves a user on a hidden model. Already verified 2026-09-06, do not re-derive: the MINIMAL thinking clamp (3.8 answers HTTP 400 to thinkingLevel minimal and 200 to low, identical to 3.7), and the migration on a real install (MIGRATION: selectedImprovementModel gemini-3.6-flash to gemini-3.8-flash, on a machine whose didMigrateImprovementTo37Flash flag was already spent — the case the first implementation missed). Longer horizon: the 2026-10 audit must rank gemini-3.8-flash at or above gemini-3.7-flash on glossary adherence (baseline: the 2026-09-03 measurements) — falsified if it ranks below. Both models are now in benchmark-transcription.py's MODELS with the MINIMAL clamp widened to cover them, so that comparison measures models rather than two HTTP 400s. | BUILD | BRANCH implementer/q4-20260906 (PR #55) |  |
| 5 | usage-review 2026-09-07 — improvement-ledger I4 / instrumentation-gaps #9 | In WhisperShortcut/SpeechService.swift add an `origin` parameter (streamingChunk \| dictation \| meetingSegment \| promptHistory) plus the audio duration to both `logNoSpeechDetected` calls at the apiEmptyResult sites (:2156, :2169), thread it from the callers including WhisperShortcut/DictateStreamingSession.swift, widen `ContextLogger.logNoSpeechDetected` in WhisperShortcut/ContextLogger.swift to carry it in `detail`, and stop hardcoding peakDb/durationMs as "n/a" on that path. | On 2026-09-14, run `cat signals-*.jsonl \| jq -r 'select(.kind=="noSpeechDetected")\|[.detail.origin,.detail.durationMs]\|@tsv' \| sort \| uniq -c` over the new review window: it did NOT work if any record still shows an empty/absent `origin` or `durationMs: "n/a"`, or if `origin=="streamingChunk"` records are still counted in the dictation no-speech rate. Baseline to beat: 139 of 182 records unattributable, 177 of 182 with `durationMs: "n/a"`. | ASK | OPEN |  |
| 6 | usage-review 2026-09-07 — improvement-ledger I6 | In WhisperShortcut/ModelManager.swift make `findFile`/`hasRequiredWhisperKitFiles` verify each required `.mlmodelc` is a loadable compiled model (non-empty directory with its expected contents) rather than only that a directory of that name exists, so an interrupted download reports the model as unavailable and is re-fetched; and in WhisperShortcut/LocalSpeechService.swift bound `initializeModel` and `transcribe` with a wall-clock deadline that surfaces the existing retryable error instead of running until the user cancels. | On 2026-09-14, `grep -c 'Model appears to be missing or incomplete\\|Unable to load model' build/usage-review-staging/errors-*.log` must be 0, and `cat signals-*.jsonl \| jq -c 'select(.kind=="cancelledWhileProcessing" and .gapMs>60000)'` must return no record within 5 minutes of a LOCAL-SPEECH error line. Baseline: 2 load failures (2026-08-31 large-v3-turbo, 2026-09-03 whisper-small) and 3 cancels of 36.7s / 313.7s / 486.8s. | BUILD | OPEN |  |
| 7 | usage-review 2026-09-07 — improvement-ledger I7 | Decide whether the glossary-echo and implausible-transcript guards in WhisperShortcut/TextProcessingUtility.swift should keep returning "" (which becomes noSpeechDetected and is skipped at WhisperShortcut/DictateStreamingSession.swift:151, dropping that chunk from the transcript) or throw a distinct error so the neighbouring `return nil` single-shot re-transcription branch runs instead. | On 2026-09-14, `grep -c 'Discarding glossary-echo transcript\\|Discarding implausible transcript' build/usage-review-staging/errors-*.log` paired with a new discard OutcomeSignal: it did NOT work if discards still occur with no corresponding single-shot fallback log line and no signal. Baseline: 9 glossary-echo discards on 2026-08-31 (all 10.8-11.6s audio) and 2 implausible discards on 2026-09-01, all on gemini-3.1-flash-lite, none visible in the signal stream. | ASK | DEFECT |  |
| 8 | usage-review 2026-09-14 — improvement-ledger I8 / instrumentation-gaps #10 | In WhisperShortcut/ContextLogger.swift add a noteProcessingStart() marker beside noteDictationStart and emit detail.processingMs (ms since processing began) on the cancelledWhileProcessing emits at WhisperShortcut/MenuBarController.swift:1319/:1379 and on the requestTimedOut emit in WhisperShortcut/SpeechService.swift; on the streaming path also emit detail.pendingChunks from WhisperShortcut/DictateStreamingSession.swift. Instrumentation only — do not change the deadline or any cancel behaviour. | On 2026-09-21 run `cat build/usage-review-staging/signals-*.jsonl \| jq -c 'select(.kind=="cancelledWhileProcessing" or .kind=="requestTimedOut") \| [.kind,.detail.processingMs,.detail.pendingChunks]'`: it did NOT work if any record lacks a numeric detail.processingMs, or if a requestTimedOut record for a single-shot dictation shows processingMs outside 55000–70000. Baseline: 8 such records this week, all with gapMs 419569–10982545 and no processingMs. | ASK | OPEN |  |
| 9 | usage-review 2026-09-14 — improvement-ledger I9 | Decide, once gap #9 (queue row 5) data says whether the audio was silent or the provider returned empty for audible input, whether WhisperShortcut/DictateStreamingSession.swift:151 should surface a pill warning after N consecutive empty chunks or fall back to single-shot at stop instead of joining N empties; blocked on queue row 5. | On 2026-09-28 run the empty-chunk episode clustering over signals-*.jsonl (consecutive noSpeechDetected < 40 s apart): it did NOT work if any episode of >= 2 consecutive apiEmptyResult still ends in a cancelledWhileProcessing with no interaction record and no warning/fallback log line. Baseline: 2 of 44 episodes this week (11 empties over 132 s on 2026-09-07 07:30Z; 2 empties on 2026-09-08 16:06Z). | ASK | DEFECT |  |
| 10 | log triage 2026-09-16 20:25 — dictation silent after deploy-8.17 worktree removed | The app launched by a release/rebuild inside a worktree (scripts/rebuild-and-restart.sh:118 opens $APP_PATH under <worktree>/build/DerivedData) keeps running after the worktree is deleted; tccd then fails proc_pidpath_audittoken for the pid, coreaudiod logs "Client is not granted access to the input device" and every dictation records -120 dB silence (6 skipped recordings 20:25–20:26 on 2026-09-16, fixed only by relaunching from the main checkout). Guard both ends: (a) in scripts/rebuild-and-restart.sh, when PROJECT_DIR is under .claude/worktrees/, print a loud warning that the running app will die with the worktree (or offer --launch-from-main to relaunch the main-checkout build instead); (b) add scripts/worktree-remove.sh (and use it from scripts/implementer/release-merges.sh:312 and run-implementer.sh:395) that refuses to remove a worktree while `pgrep -f "<worktree>/build/DerivedData"` finds a live WhisperShortcut process, printing the pid and the relaunch command. | On the next release cut from a worktree, `log show --predicate "process == \"tccd\""` contains no "proc_pidpath_audittoken() failed from PID" line for WhisperShortcut and no dictation logs `peak -120.0 dB` while a worktree is removed; if the warning/refusal never fires in 30 days because nobody releases from a worktree, mark NO DATA not NO EFFECT. | BUILD | OPEN |  |
| 11 | log triage 2026-09-16 20:25 — six consecutive -120 dB recordings skipped silently | ChunkedDictateRecorder (WhisperShortcut/ChunkedDictateRecorder.swift:130) reports peak -120.0 dB when CoreAudio delivers all-zero input (TCC denial, dead binary path, unplugged device) and MenuBarController.swift:2752 treats it like a quiet user ("Skipping API call — recording was silent", noSpeechDetected signal). Digital zero for a whole ≥2 s recording is never a quiet user. Distinguish it: when lastPeakPowerDb <= -100 (exact digital silence) show the bottom-center error popup "Microphone delivered no audio — check System Settings ▸ Privacy ▸ Microphone or restart WhisperShortcut" instead of the silent skip, log a distinct AUDIO: No input signal line, and emit a new ContextLogger signal (noInputSignal) so usage-review can count it separately from noSpeechDetected. Keep the existing quiet-user path for -100 < peak < -45. | After release, the interactions JSONL shows noInputSignal rows only when the unified log has a matching coreaudiod "not granted access to the input device" or device-change line in the same minute, and no genuine quiet-user recording (peak between -100 and -45 dB) triggers the popup in 30 days of the operator's own usage; zero noInputSignal rows in 30 days is NO DATA, not NO EFFECT. | VETO | OPEN | 2026-09-23 |
