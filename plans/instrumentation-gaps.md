...
| 6 | Apple's analytics reports are pulled ad-hoc; the install/delete/session series is not part of any scheduled run | growth-review's retention and activation factors — it graded them "unmeasured" in G1 although the data existed | OPEN | Recipe verified 2026-08-18 (below). Add it to `review-growth` Phase 1 so every run reads churn instead of assuming it. **Widened 2026-09-05 (G2): the endpoints are now a reliability risk, not just an omission.** At `ASC_TIMEOUT=240s`, `r15` (Discovery and Engagement Detailed) would not enumerate its instances at all — three attempts, all `context deadline exceeded` — and 4 of 11 `r12` purchase instances plus 30 of 34 `r3` download instances never downloaded, so G2's August purchase count (3) is a floor. Two further traps found while deduplicating: a MONTHLY instance may hold the **whole history** rather than one month (G1 double-counted July 2× from two overlapping instances), and a DAILY instance's `processingDate` is 1–3 days AFTER the dates inside it. Always dedupe by the `Date` column, latest processing instance wins. |
...
| 8 | `TranscriptionError.noSpeechDetected` writes nothing to the outcome-signal stream — 39 occurrences in `errors-*.log` (2026-07-22…2026-08-20) against **0** records in `interactions-*.jsonl`. Neither the local silence gate (`MenuBarController.swift:2432`, skips the API call) nor the API-empty path (`SpeechService.swift:2002`) emits an `OutcomeSignal`, and the discriminating fields (peak dB, recording duration) are computed then discarded | usage-review cannot rate the dictation no-speech failure mode at all, and **no fix to it can be falsified** — which is why improvement-ledger I3 has sat `proposed` since run 2 | PARTIAL | **BUILT 2026-09-02** on `claude/app-improvement-plan-4c3a3e`. `OutcomeSignal.noSpeechDetected` plus `ContextLogger.logNoSpeechDetected` from the local silence gate (`source: localSilenceGate`) and `validateSpeechText` empty path (`source: apiEmptyResult`). **Downgraded to PARTIAL 2026-09-07 (usage-review run 4): it emits, but the emitted data cannot answer the question.** 182 signals in 6 days, of which 139 carry only the provider constant `OPENAI-TRANSCRIPTION` as `logPrefix` and are (by their 11.0s inter-arrival period) benign `ChunkedDictateRecorder` chunk empties, and 177 of 182 log `peakDb`/`durationMs` as `"n/a"`. See gap #9. |
| 9 | `noSpeechDetected` records cannot be attributed to a caller and carry no discriminating data: `SpeechService.swift:2156`/`:2169` pass the *provider* constant (`OPENAI-TRANSCRIPTION`) as `logPrefix` and hardcode `peakDb: "n/a"`, `durationMs: "n/a"`. So a benign streaming-dictate chunk pause (skipped at `DictateStreamingSession.swift:151`, dictation still succeeds) is indistinguishable from a whole dictation that returned nothing | usage-review's read of the dictation no-speech failure mode — the same thing gap #8 was opened for. Measured 2026-09-07: 139 of 182 signals are unattributable, so the apparent 23.5% no-speech rate is unusable | OPEN | Opened by usage-review run 4 (2026-09-07), improvement-ledger I4. Fix: thread a caller-supplied origin (`streamingChunk`/`dictation`/`meetingSegment`/`promptHistory`) plus the chunk duration into both `apiEmptyResult` emit sites, and tag or suppress streaming-chunk empties. Closes when every record in a window carries a non-`n/a` origin and duration and the daily count stops tracking dictation length. |
| 10 | `cancelledWhileProcessing` and `requestTimedOut` carry no elapsed time for the stalled interaction: `logSignal` defaults `refTs` to `lastEntryTs[mode]` (the previous *completed* interaction), so `gapMs` is the distance to the last dictation, not the wait — 10 982 545 ms (3.05 h) on 2026-09-07. `MenuBarController.swift:1319` passes no `refTs`; `ContextLogger.noteDictationStart` stamps a start marker but nothing stamps the stop | usage-review's grade of improvement-ledger I2 / queue row 1 (`gapMs > 120000` has never measured a wait) and any read of whether a cancel in `transcribing` happened inside or outside the 60 s NetworkDeadline | OPEN | Opened by usage-review run 5 (2026-09-14), improvement-ledger I8. Fix: record the processing-start instant when recording stops, emit `detail.processingMs` on both signals and `detail.pendingChunks` on the streaming path. Closes when every such record in a window carries a numeric `processingMs`. |
