...
| I5 | **Dictate Prompt presets** (ledger I5, reinforced in run 5): one-tap Correct / Format / Rephrase so short command verbs are not voice-dictated through the STT that garbles them. | ledger | **Reverted 2026-09-04.** Shipped inside the Sept-2 plan commit without a separate approval. Not one-tap (still starts a recording), no shortcut, and the owner rejected the menu clutter. Do not re-ship. | M |
