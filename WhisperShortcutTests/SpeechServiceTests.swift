...
final class SpeechServiceTests: XCTestCase {
  ...
  func testAPIKeyValidationWithEmptyKey() async {
    do {
      _ = try await speechService.validateAPIKey("")
  ...
  func testAPIKeyValidationWithInvalidKey() async {
    do {
      _ = try await speechService.validateAPIKey("sk-invalid-test-key")
  ...
  func testModelSelection() {
    // Test default model
    XCTAssertEqual(
      speechService.getCurrentModel(), .gpt4oMiniTranscribe,
      "Default model should be gpt4oMiniTranscribe")

    // Test setting different models
    speechService.setModel(.gpt4oMiniTranscribe)
    XCTAssertEqual(
      speechService.getCurrentModel(), .gpt4oMiniTranscribe,
    ...
    XCTAssertEqual(
      speechService.getCurrentModel(), .gpt4oTranscribe,
