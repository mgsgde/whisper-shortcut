...
description: Systematically reviews all LLM-context files (.cursor/commands/, .cursor/rules/, .cursor/skills/, CLAUDE.md) for stale references, factual drift vs the current codebase, redundancy, and files that no longer earn their slot. Produces a tiered report.
---

# Audit LLM-Context Files

Run a thorough review of every markdown file that gets loaded into LLM prompts: cursor commands, cursor rules, cursor skills (symlinked into `.claude/skills/`), and the top-level `CLAUDE.md`. Goal: find files that have silently rotted (broken references, function pointers at wrong lines, factual drift, redundancy across files).
...
- `--scope=commands|rules|skills|claude-md` — narrow to one group.
...
1. Dispatch parallel subagents (Agent tool, `subagent_type: general-purpose`), one per group: commands, rules, skills. Each agent reads its files, spot-verifies claims against the codebase, and reports per-file findings with severity tags + cited `file:line`.
2. While they run, the orchestrator does the **cross-cutting checks** that need a global view: single-source-of-truth between `CLAUDE.md` and `.cursor/rules/`, skill ↔ bash-script overlap, hardlink/symlink integrity between `.cursor/skills/` and `.claude/skills/`, naming-convention adherence vs `.cursor/commands/README.md`.
