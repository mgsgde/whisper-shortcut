...
- Cloud features use your Google Gemini API key and, optionally, an xAI API key (Grok chat and TTS), an OpenAI API key (GPT-5.x chat, transcription, GPT Audio Dictate Prompt, TTS), or an Anthropic API key (Claude chat). Offline Whisper needs no key at all.
...
- **Chat**: Use a persisted multi-session chat window with Gemini, Grok, OpenAI, Claude (Anthropic), or offline MLX models, screenshots, image attachments, slash commands, optional web grounding, and per-session reasoning depth. Share a folder with `/folder` and the chat can read it — including the `AGENTS.md` / `CLAUDE.md` files and the rules and skills under `.agents/`, `.cursor/`, or `.claude/` you already keep there, which are loaded into every message automatically so the chat works from your own context. Turn on file editing in Settings and it can write notes back into that folder too. Grok models additionally search X.com posts, which makes them the best pick for opinions, trends, and breaking social-media chatter — and `/x @handle` narrows that search to the accounts you care about.
- **Your own endpoint (Azure OpenAI, Vertex AI, or any OpenAI-compatible proxy)**: Point chat at a URL you control instead of a vendor the app picked — an OpenRouter/LiteLLM/OpenInference proxy, or your own **Azure OpenAI / Microsoft Foundry** resource or **Google Vertex AI** project. Enter the base URL, the model (on Azure: your *deployment* name), and an API key in Settings → Chat; one-click presets fill in the URL shape for each. Azure is detected from the URL and its key is sent the way Azure expects (`api-key`, not a bearer token), a bare Azure resource URL is expanded to `/openai/v1`, and an `api-version` is filled in for a classic deployment URL that omits it. Dictate can use the same tenant through the self-hosted transcription endpoint, which now speaks Azure too. Requests go straight from your Mac to that endpoint, in the region your resource or project runs in, under your own Microsoft or Google contract — no server of ours is involved. That is a statement about where inference runs and who your data-processing agreement is with; it is not a certification of this app, and whether it satisfies a professional confidentiality duty is a call for you and your counsel. Vertex takes a short-lived `gcloud auth print-access-token` value as the key, so it has to be pasted again about hourly; Azure keys do not expire. For dictation that never leaves the Mac at all, use **Offline Mode** instead.
...
- Optional Anthropic API key for Claude chat models
...
Model shortcuts include `/gemini`, `/grok`, `/gpt`, `/openai`, `/claude`, `/anthropic`, and per-model aliases such as `/gemini35flash`. Gemini models use your Google API key, Grok models use your xAI API key, OpenAI models use your OpenAI API key, and Claude models use your Anthropic API key.

Connect Google or Trello in Settings → Chat to unlock the corresponding chat tools. Creating or deleting a calendar event, changing a Trello card, or opening a URL asks for confirmation first.
...
- **Copy transcript** in the meeting bar puts the raw transcript on the clipboard; right-click it to reveal the file in Finder. **Copy transcript + chat** next to it copies the transcript followed by every question and answer from the meeting chat, as one Markdown document.
...
With **Offline Mode** on (Settings → Privacy & Permissions), requests the app itself builds may not leave your Mac and the usage log is not written at all — no transcripts, prompts, replies or audio samples on disk. WhisperKit model downloads still reach Hugging Face through WhisperKit's own URLSession. Note that auto-paste needs the Accessibility permission and is therefore only available in the direct-download build, not the App Store one.
